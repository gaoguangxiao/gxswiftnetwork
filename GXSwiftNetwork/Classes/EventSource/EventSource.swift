...
public protocol EventSourceProtocol {
    ...
    /// - Parameter onOpenCallback: callback
    func onComplete(_ onComplete: @escaping ((Int?, Bool?, Error?, String?) -> Void)) -> Self
    
    /// This callback is called everytime an event with name "message" or no name is received.
    func onMessage(_ onMessageCallback: @escaping ((_ id: String?, _ event: String?, _ data: String?) -> Void)) -> Self
...
open class EventSource: NSObject, EventSourceProtocol, URLSessionDataDelegate {
    ...
    private(set) public var bodyJSON: String?
    ...
    private var onComplete: ((Int?, Bool?, Error?, String?) -> Void)?
    ...
    public func connect(lastEventId: String? = nil) {
        ...
        } else if let bodyData = bodyJSON?.data(using: .utf8) {
            var request = URLRequest(url: url)
    ...
    public func onOpen(_ onOpenCallback: @escaping (() -> Void)) -> Self {
        self.onOpenCallback = onOpenCallback
        return self
    }
    
    public func onComplete(_ onComplete: @escaping ((Int?, Bool?, Error?, String?) -> Void)) -> Self{
        self.onComplete = onComplete
        return self
    }
    
    public func onMessage(_ onMessageCallback: @escaping ((_ id: String?, _ event: String?, _ data: String?) -> Void)) -> Self{
...
//            print("所有响应头: \(allHeaders)")
            // 获取特定 X-Tt-Logid 头
            let key = "X-Tt-Logid"
            if #available(iOS 13.0, *) {
                if let logId = httpResponse.value(forHTTPHeaderField: key) {
                    //                print("X-Tt-Logid: \(logId)")
                    if let eventHandler = eventListeners[key] {
                        mainQueue.async { eventHandler("event.id", "", logId) }
                    }
                } else {
                    print("响应头中没有找到 X-Tt-Logid")
                }
            } else {
                // Fallback on earlier versions
                if let logId = httpResponse.allHeaderFields[key] {
                    //                print("X-Tt-Logid: \(logId)")
                    if let eventHandler = eventListeners[key] {
                        mainQueue.async { eventHandler("event.id", "", logId as? String) }
                    }
                } else {
                    print("响应头中没有找到 X-Tt-Logid")
                }
    ...
    open func urlSession(_ session: URLSession,
        ...
        guard let responseStatusCode = (task.response as? HTTPURLResponse)?.statusCode else {
            mainQueue.async { [weak self] in self?.onComplete?(nil, nil, error, nil) }
            return
        }
        
        let reconnect = shouldReconnect(statusCode: responseStatusCode)
        mainQueue.async { [weak self] in guard let self else { return }
            
            if let dataBuffer = eventStreamParser?.dataBuffer,
               dataBuffer.count > 0,
               let dataStr = String(data: Data(referencing: dataBuffer), encoding: .utf8) {
                onComplete?(responseStatusCode, reconnect, nil, dataStr)
            } else {
                onComplete?(responseStatusCode, reconnect, nil, nil)
            }
        }
    }
...
//        additionalHeaders["Accept"] = "text/event-stream"
        ...
        sessionConfiguration.timeoutIntervalForResource = 20.0 //整个资源的超时（含重试）
